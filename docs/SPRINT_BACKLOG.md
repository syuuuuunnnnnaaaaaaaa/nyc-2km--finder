...
| **Sprint 5** | E2E 테스트, 성능 최적화, 접근성 및 배포 | 🟢 완료 | 100% |
...
| 태스크 ID | 작업 내용 | 우선순위 | 상태 | 산출물/비고 |
|---|---|:---:|:---:|---|
| S5-01 | 유효성 검사, 타임아웃, 동선 계산 단위 테스트 작성 (20개 테스트 100% 통과) | High | ✅ 완료 | `tests/` |
| S5-02 | 스크린 리더(`aria-*`, `role="alert"`) 웹 접근성(a11y) & SEO 강화 | Medium | ✅ 완료 | `app/layout.tsx`, UI 전반 |
| S5-03 | 프로덕션 빌드 검증 및 린트 검사 (`npm run build`) | High | ✅ 완료 | Next.js Turbopack |
| S5-04 | 릴리즈 노트 및 배포 준비 완료 | High | ✅ 완료 | `docs/RELEASE_NOTES.md` |
